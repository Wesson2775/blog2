...
export default function SearchModal({ open, onClose, onSearch }: { open: boolean, onClose: () => void, onSearch: (q: string) => void }) {
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <button className="absolute top-4 right-4 text-2xl text-white" onClick={onClose} aria-label="关闭">×</button>
      <form
      ...
      >
        <div className="flex items-center w-full" style={{ width: '80vw', maxWidth: 400 }}>
          <input
            ...
            placeholder="搜索文章"
            className="flex-1 bg-transparent border-0 outline-none text-white text-lg placeholder:text-gray-400 px-0 focus:ring-0 focus:outline-none"
            style={{ boxShadow: 'none', borderBottom: 'none' }}
          />
          <button type="submit" className="w-10 h-10 flex items-center justify-center rounded-full bg-red-500 hover:bg-red-600 transition-colors ml-2" title="搜索">
